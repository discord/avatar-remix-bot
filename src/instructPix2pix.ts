...
export async function instructPix2PixHandler(request: IRequest, env: Env) {
  ...
  if (!jobString) {
    console.error(`Received instructpix2pix callback for unknown replicateId ${replicateId}`);
...
export async function startInstructPix2Pix(job: Job, env: Env) {
  ...
  const replicatePayload = {
    ...
    input: {
      image: job.url,
      prompt: job.prompt,
      negative_prompt: USE_NEGATIVE_PROMPT ? job.negativePrompt || NEGATIVE_PROMPT : undefined,
