...
export async function sendResult(replicateId: string, imageUrl: string, job: Job, env: Env) {
  // Record followup with output URL, so that it can be remixed.
  await env.AVATAR_REMIX_FOLLOWUPS.put(`replicateId:${replicateId}`, JSON.stringify(job), {
  ...
  const hasTargetBeenPinged = await env.AVATAR_REMIX_FOLLOWUPS.get(`pinged:${job.targetUserId}`);
